...
def configurar_treeview(treeFrame):
...
def configurar_botoes(widgets_frame, insert_callback, toggle_callback):
    button = ttk.Button(widgets_frame, text="Insert", command=insert_callback)
    button.grid(row=4, column=0, padx=5, pady=5, sticky="nsew")

    separator = ttk.Separator(widgets_frame)
    separator.grid(row=5, column=0, padx=(20, 10), pady=10, sticky="ew")

    mode_switch = ttk.Checkbutton(widgets_frame, text="Mode", command=toggle_callback)
...
treeview = configurar_treeview(treeFrame)
button, mode_switch = configurar_botoes(widgets_frame, insert_row, toggle_mode)
